...
class EvalTMVA:
    ...
    def EvaluateFNN(self):
        ...
        for Name in self.ModelNames:
            ...
            OutPreOther, OutPreSame = self.ImportXml(train,test,Name)
            np.savetxt(self.SavePath+Name+"_OutPreOther.txt",OutPreOther)                                                 
            np.savetxt(self.SavePath+Name+"_OutPreSame.txt",OutPreSame)
